...
public abstract class AbstractGUI {
	...
	protected void drawTexturedRect(int x, int y, int u, int v, int width, int height) {
		...
		buffer.pos((double)x, (double)(y + height), (double)this.zLevel).tex((double)((float)u * f), (double)((float)(v + height) * f1)).endVertex();
		buffer.pos((double)(x + width), (double)(y + height), (double)this.zLevel).tex((double)((float)(u + width) * f), (double)((float)(v + height) * f1)).endVertex();
		buffer.pos((double)(x + width), (double)y, (double)this.zLevel).tex((double)((float)(u + width) * f), (double)((float)v * f1)).endVertex();
		buffer.pos((double)x, (double)y, (double)this.zLevel).tex((double)((float)u * f), (double)((float)v * f1)).endVertex();
		tessellator.draw();
	}
...
//		TODO: Fix this, GUI components don't show up behind transparent rects
	...
	protected void drawGradientRect(int x, int y, int width, int height, Color topColor, Color bottomColor) {
		...
		buffer.pos((double) x + width, (double) y, (double)this.zLevel).color(topColor.getRed(), topColor.getGreen(), topColor.getBlue(), topColor.getAlpha()).endVertex();
		buffer.pos((double) x, (double) y, (double)this.zLevel).color(topColor.getRed(), topColor.getGreen(), topColor.getBlue(), topColor.getAlpha()).endVertex();
		buffer.pos((double) x, (double) y + height, (double)this.zLevel).color(bottomColor.getRed(), bottomColor.getRed(), bottomColor.getBlue(), bottomColor.getAlpha()).endVertex();
		buffer.pos((double) x + width, (double) y + height, (double)this.zLevel).color(bottomColor.getRed(), bottomColor.getRed(), bottomColor.getBlue(), bottomColor.getAlpha()).endVertex();
		tessellator.draw();
