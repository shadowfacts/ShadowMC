...
val IInventory.isEmpty: Boolean
	get() {
		forEach {
			if (it !== null) {
				return false
...
inline fun IInventory.firstOrNull(predicate: (ItemStack?) -> Boolean): ItemStack? {
	forEach {
		if (predicate(it)) {
			return it
		}
...
inline fun <R: Comparable<R>> IInventory.maxBy(selector: (ItemStack?) -> Int): ItemStack? {
	if (isEmpty) return null
	var maxElem = this[0]
